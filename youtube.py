...
def get_video_metadata(url: str, output_dir: str) -> Dict[str, Any]:
    ...
    and downloads the MP4 video using yt-dlp with cloud datacenter fallback.
    """
    os.makedirs(output_dir, exist_ok=True)
    video_id = extract_video_id(url)
    
    # Try yt-dlp first with android/web client fallback
    ydl_opts = {
    ...
    try:
        ...
        print(f"[YouTube Metadata Warning] yt-dlp direct fetch failed on cloud IP: {e}")
        # Fallback to YouTube oEmbed API (Never bot-blocked on Cloud Datacenters!)
        try:
            ...
            print(f"[YouTube oEmbed Error]: {oembed_err}")
